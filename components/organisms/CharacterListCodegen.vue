...
import { Character, Characters, useGetAllCharactersQuery, useGetCharacterDetailsQuery } from '~/@types/generated'

const characters = ref<Characters['results']>([])
const pagination = ref<Characters['info']>({
  pages: 1,
  count: 1,
  next: 1,
  prev: 1,
...
const { onResult } = useGetAllCharactersQuery(
  () => ({
...
const { onResult: onResultDetail } = useGetCharacterDetailsQuery(() => ({ id: details.id }), () => ({ enable: details.enable, fetchPolicy: 'cache-and-network' }),)

onResultDetail(({ data, networkStatus }) => {
  console.group('[CODEGEN] CHARACTER DETAIL')
  if (networkStatus === 7) {
